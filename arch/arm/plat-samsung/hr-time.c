/*
 * linux/arch/arm/plat-samsung/hr-time.c
 *
 * S3C6410 Timers
 *
...
static void s3c64xx_tick_timer_start(unsigned long load_val, int autoreset)
{
	unsigned long tcon;
	unsigned long tcnt;
	unsigned long tcfg1;
...
static void s3c64xx_sched_timer_start(unsigned long load_val, int autoreset)
{
	unsigned long tcon;
	unsigned long tcnt;
	unsigned long tcfg1;
...
static void __init  s3c64xx_init_dynamic_tick_timer(unsigned long rate)
{
	...
	printk("High resolution timer: rate = %ld, HZ = %d\n", rate, HZ);
...
static void __init s3c64xx_init_clocksource(unsigned long rate)
{
	...
	clocksource_s3c64xx.mult = clocksource_khz2mult(rate/1000, clocksource_s3c64xx.shift);
