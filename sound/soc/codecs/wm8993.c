...
static int configure_clock(struct snd_soc_codec *codec)
{
	...
	printk("wm8993->sysclk_source: %d\n",wm8993->sysclk_source);
...
{
	printk_wei("Into wm8993_hw_params");
	struct snd_soc_codec *codec = dai->codec;
	struct wm8993_priv *wm8993 = snd_soc_codec_get_drvdata(codec);
...
printk_wei("wm8883_hw_param success");
	return 0;
}
...
printk_wei("Into wm8993_probe success return\n");
	return ret;

err:
printk_wei("Into wm8993_probe error label\n");
	return ret;
}
			    ...
			    const struct i2c_device_id *id)
{
	...
	printk_wei("Into wm8993_i2c_probe\n");
	...
	printk("Read Reset return value: 0x%xh\n",val);
...
printk("Write Reset returned value: 0x%xh\n",ret);
printk("Write Reset and reread value: 0x%xh\n",val);
///////////// END OF ADD /////////////
	codec->cache_only = 1;
	...
	if (ret != 0)
		goto err_enable;
	printk("Set SND_SOC_BIAS_STANDBY return value: 0x%xh\n",ret);
	...
	printk_wei("Out wm8993_i2c_probe success\n");
	return 0;
...
static int __init wm8993_modinit(void)
{
	printk("----> %s <----\n",__FUNCTION__);
	int ret;
	printk_wei("Into wm8993_modinit\n");
	ret = i2c_add_driver(&wm8993_i2c_driver);
	if (ret != 0)
		pr_err("WM8993: Unable to register I2C driver: %d\n", ret);
	printk_wei("Out wm8993_modinit with return value: %d\n",ret);
