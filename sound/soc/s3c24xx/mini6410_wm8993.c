...
/* SMDK64XX has a 12MHZ crystal attached to WM8993 */
#define SMDK64XX_WM8993_FREQ 12000000

static int smdk64xx_hw_params(struct snd_pcm_substream *substream,
	struct snd_pcm_hw_params *params)
{
	
	printk_wei("Into smdk64xx_hw_params\n");
...
*/
	...
	if (ret < 0)
		return ret;
printk_wei("Out of smdk64xx_hw_params\n");
	return 0;
}
...
static int __init smdk64xx_audio_init(void)
{
	...
	printk_wei("Into smdk64xx_audio_init\n");
	printk_wei("Start register platform device\n");

	smdk64xx_snd_device = platform_device_alloc("soc-audio", -1);
	if (!smdk64xx_snd_device)
	{
		
		printk("Alloc smdk64xx_snd_device faild!\n");
		return -ENOMEM;
	}

	printk_wei("Alloc smdk64xx_snd_device successfully!\n");
