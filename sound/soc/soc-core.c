...
{
printk_wei("##Into snd_soc_instantiate_card##\n");
	...
	if (!ac97)
		for (i = 0; i < card->num_links; i++) {
			...
			list_for_each_entry(dai, &dai_list, list){
				printk("dai->name: %s\n",dai->name);
				printk("card->dai_link[%d].codec_dai->name --> %s\n",i,card->dai_link[i].codec_dai->name);
			...
			if (!found) {
				printk("i=%d  DAI %s not registered\n",i,card->dai_link[i].codec_dai->name);
				dev_dbg(card->dev, "DAI %s not registered\n",
...
printk_wei("Start register snd card\n");
	...
	printk("##snd_soc_instantiate_card Success##\n");
	return;
...
{
printk_wei("Into snd_soc_instantiate_cards\n");
...
static int snd_soc_register_card(struct snd_soc_card *card)
{
printk_wei("Into snd_soc_register_card\n");
...
printk_wei("snd_soc_register_card successfully\n");
...
#endif
	int tmp = platform_driver_register(&soc_driver);

printk_wei("soc_driver platform_driver_register returned value is : %d\n",tmp);
	return tmp;
}
